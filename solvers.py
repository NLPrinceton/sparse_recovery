...
def NonnegativeBP(A, b, x0=None, tol=1E-4, niter=100, biter=32):
  ...
  for i in range(niter):
    ...
    for j in range(biter):
      ...
      vp = v+s*dv
      s *= beta
      if all(xp>0.0) and np.sqrt(norm(1.0-lamp+AT.dot(vp))**2+norm(lamp*xp-oot)**2+norm(A.dot(xp)-b)**2) <= (1.0-alpha*s)*resnorm:
        break
    else:
      break
...
if __name__ == '__main__':
  ...
  d = 50
  n = 500
  A = (2.0*np.random.randint(2, size=(d, n))-1.0) / np.sqrt(d)
  ATinvAAT = A.T.dot(inv(A.dot(A.T)))
  x = np.zeros(n)
  tol = 1E-4
